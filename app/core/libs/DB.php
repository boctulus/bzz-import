	...
	*/
	public static function beginTransaction(){
		...
		try {
        	...
        	static::$inited_transaction = false; // fixed by chatGTP on 1-dic-2023
    	}		
	}
	...
	public static function rollback(){
		...
		static::getConnection()->rollback(); // line 538
		static::$inited_transaction = false;
	}
		...
		try {
			...
			if ($result){
				...
				if (isset($data[$id_name])){
					$last_inserted_id =	$data[$id_name]; // error
				} else {
		...
		try {
			...
			{				
				...
				elseif(is_resource($val))	
					...
					$type = \PDO::PARAM_LOB;  // 3
				elseif(is_array($val)){
					throw new \Exception("The value for WHERE can not be an array!");				
				}else {
