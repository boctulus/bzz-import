...
class Products extends Posts
{
    ...
    */
    ...
    // static function getProductIDBySKU_V1($sku, $post_status = null){
    ...
    //     return count($result_ay) == 1 ? $result_ay[0]['ID'] : array_column($result_ay, 'ID');
    // }    
    ...
    */
    ...
    static function deleteImagesByPID($post_id){
    ...
    */  
    ...
    /*
        ...
        @param $pid post-id
        @param int|string|Array $catego la categoria ya sea el term_id, slug o Array de estos
    */
    static function setProductCategory($pid, $catego){
        if (is_array($catego) && count($catego) >0){
            foreach ($catego as $cat){
                if (is_array($cat)){
                    $cat = $cat['name'] ?? $cat['slug'] ?? null;
                }

                if (empty($cat)){
                    throw new \InvalidArgumentException("Invalid format for category");
                }

                wp_set_object_terms($pid, $cat, static::$cat_metakey);
            }
        } else {
            wp_set_object_terms($pid, $catego, static::$cat_metakey);
        }
    }
    ...
    */
    static function addProductCategoryNames($pid, Array $categos){
        ...
        static::setProductCategory($pid, $categos);
    }
        ...
        */
        ...
        if( isset( $args['categories'] ) ){
            if (is_numeric($args['categories'][0])){
                foreach ($args['categories'] as $cat_id){
                    wp_set_object_terms($pid, $cat_id, 'product_cat');
                }
            } else {
                static::setProductCategory($pid, $args['categories']);
            }            
        }               
