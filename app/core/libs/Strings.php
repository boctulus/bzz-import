...
{	
	...
	*/
	...
	/*
		...
		foreach ($pids as $pid){
            ...
            $tit = $product->get_name();
            $tit = str_replace('-', ' ', $tit);

            if (Strings::containsAnyWord(['Sneaker', 'Sneakers', "Shoes", "Trainers", "Scarpe", "Stivaletti"], $tit)){

                if (Strings::containsWords([
                    "Alexander Mcqueen", "Uomo"
                ], $tit, false) || Strings::contains("Tread Slick", $tit, false)){
            
                    dd("Actualizando categoria para '$tit' ($pid)");

                    Products::setProductCategory($pid, 'alexander-mcqueen-scarpe-da-uomo');
                }
                ...
                if (Strings::containsWords([
					...
					Products::setProductCategory($pid, 'versace-scarpe-da-uomo');
                }
            }
        }
	*/
	...
	*/
	...
	static function removeTrailingSlash($path){
		return rtrim($path, "/\\");
	}
